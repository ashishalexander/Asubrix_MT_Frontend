...
import { Row, Col, Card, Badge, Dropdown, Form, Button, Modal, Breadcrumb, Spinner, Alert } from 'react-bootstrap';
import { 
...
const TestsPortal = ({ 
  ...
  const filteredAndSortedTests = tests?.length ? sortItems(
    tests.filter(test => 
                ...
                <Card.Body>
                  <div className="d-flex align-items-center">
                    ...
                    <div className="content-details flex-grow-1">
                      ...
                      <div className="content-meta text-muted small">
                        <div><FiClock className="me-1" />{test.duration_minutes} minutes</div>
                        <div><FiUsers className="me-1" />{test.attempts_count || 0} attempts</div>
                        <div><FiCalendar className="me-1" />{new Date(test.updated_at || test.created_at).toLocaleDateString()}</div>
                      </div>
                    </div>
                    <Dropdown>
                      <Dropdown.Toggle variant="link" className="btn-actions text-muted">
      ...
      <div className="content-header mb-4">
        {!searchResults && (
          <Breadcrumb className="mb-0">
            ...
            {currentPath.length > 0 && getBreadcrumbItems().slice(1).map((item, index) => (
              <Breadcrumb.Item
                key={item.id}
